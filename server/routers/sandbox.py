...
from ..services.sandbox_manager import SandboxInfo, get_sandbox_manager
...
async def list_sandboxes(project_name: Optional[str] = None):
    """
...
async def cleanup_stopped_sandboxes(hours: int = 24):
    ...
    return ActionResponse(
        success=True,
        message=f"Cleaned up {count} stopped sandbox(es)"
    )
