...
class AgentTracker:
    ...
    async def _handle_agent_complete(
        self, feature_id: int, is_success: bool, agent_type: str = "coding"
    ) -> dict | None:
    ...
    async def process_line(self, line: str) -> dict | None:
        ...
        async with self._lock:
            ...
            if ORCHESTRATOR_PATTERNS["init_start"].search(line):
                ...
                update = self._create_update("init_start", "Initializing project features...")

            # Check for initializer complete
            elif ORCHESTRATOR_PATTERNS["init_complete"].search(line):
                self.state = "scheduling"
                update = self._create_update("init_complete", "Initialization complete, preparing to schedule features")
            ...
            # Check for capacity status
                ...
                update = self._create_update(
                    "capacity_check", f"{self.ready_count} features ready, {slots} slots available"
                )

            # Check for at capacity
            elif ORCHESTRATOR_PATTERNS["at_capacity"].search(line):
                self.state = "monitoring"
                update = self._create_update("at_capacity", "At maximum capacity, monitoring active agents")
            ...
            # Check for coding agent spawn
                ...
                self.state = "spawning"
                update = self._create_update(
                    "coding_spawn", f"Spawned coding agent for Feature #{feature_id}", feature_id=feature_id
                )
            ...
            # Check for testing agent spawn
                ...
                self.state = "spawning"
                update = self._create_update(
                    "testing_spawn", f"Spawned testing agent for Feature #{feature_id}", feature_id=feature_id
                )
            ...
            # Check for coding agent complete
                ...
                if "testing" not in line.lower():
                    ...
                    self.state = "monitoring"
                    update = self._create_update(
                        "coding_complete", f"Coding agent finished Feature #{feature_id}", feature_id=feature_id
                    )
            ...
            # Check for testing agent complete
                ...
                self.state = "monitoring"
                update = self._create_update(
                    "testing_complete", f"Testing agent finished Feature #{feature_id}", feature_id=feature_id
                )
            ...
            # Check for all complete
                ...
                self.coding_agents = 0
                self.testing_agents = 0
                update = self._create_update("all_complete", "All features complete!")

            return update

    def _create_update(
        self, event_type: str, message: str, feature_id: int | None = None, feature_name: str | None = None
    ) -> dict:
