...
class SandboxManager:
    ...
    async def create_sandbox(
        ...
        async with self._lock:
            ...
            cmd = [
                ...
                "-d",  # Detached
                "--name", container_name,
                "--label", f"autocoder.sandbox=true",
                "--label", f"autocoder.project={project_name}",
                "--label", f"autocoder.sandbox_id={sandbox_id}",
                "--memory", memory_limit,
                "--cpus", cpu_limit,
                "--network", "autocoder_default",  # Connect to autocoder network
    ...
    async def list_sandboxes(self, project_name: Optional[str] = None) -> List[SandboxInfo]:
        """
    ...
    async def start_sandbox(self, sandbox_id: str) -> bool:
        ...
        async with self._lock:
            ...
            try:
                await self._run_command([
                    "docker", "start", info.container_id
                ])
                info.status = "running"
    ...
    async def copy_to_sandbox(
        ...
        try:
            await self._run_command([
                "docker", "cp",
                str(local_path),
                f"{info.container_id}:{sandbox_path}"
            ])
            return True
        except subprocess.CalledProcessError as e:
