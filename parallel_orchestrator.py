...
def _dump_database_state(session, label: str = ""):
    ...
    debug_log.log(
        ...
        pending_ids=[f.id for f in pending[:10]],
    )  # First 10 pending only
...
class ParallelOrchestrator:
    ...
    def get_resumable_features(self) -> list[dict]:
        ...
        try:
            ...
            stale = session.query(Feature).filter(Feature.in_progress == True, Feature.passes == False).all()
    ...
    def _maintain_testing_agents(self) -> None:
        ...
        while True:
            ...
            with self._lock:
                ...
                debug_log.log(
                    "TESTING", f"Spawning testing agent ({spawn_index}/{desired})", passing_count=passing_count
                )
    ...
    def _spawn_coding_agent(self, feature_id: int) -> tuple[bool, str]:
        ...
        threading.Thread(target=self._read_output, args=(feature_id, proc, abort_event, "coding"), daemon=True).start()
    ...
    def _spawn_testing_agent(self) -> tuple[bool, str]:
        ...
        with self._lock:
            ...
            if current_testing_count >= self.max_concurrency:
                debug_log.log(
                    "TESTING", f"Skipped spawn - at max testing agents ({current_testing_count}/{self.max_concurrency})"
                )
        ...
        threading.Thread(
            target=self._read_output, args=(feature_id, proc, threading.Event(), "testing"), daemon=True
        ).start()

        debug_log.log(
            "TESTING",
            f"Successfully spawned testing agent for feature #{feature_id}",
            pid=proc.pid,
    ...
    async def _run_initializer(self) -> bool:
        ...
        debug_log.log("INIT", "Starting initializer subprocess", project_dir=str(self.project_dir))
        ...
        try:
            ...
            debug_log.log(
                "INIT", "TIMEOUT - Initializer exceeded time limit", timeout_minutes=INITIALIZER_TIMEOUT // 60
            )
            result = kill_process_tree(proc)
            debug_log.log(
                "INIT",
                "Killed timed-out initializer process tree",
                status=result.status,
                children_found=result.children_found,
            )
            return False

        debug_log.log(
            "INIT", "Initializer subprocess completed", return_code=proc.returncode, success=proc.returncode == 0
        )
    ...
    def _on_agent_complete(
        ...
        if agent_type == "testing":
            ...
            debug_log.log(
                "COMPLETE",
                f"Testing agent for feature #{feature_id} finished",
                pid=proc.pid,
                feature_id=feature_id,
                status=status,
            )
        ...
        debug_log.log(
            "COMPLETE",
            f"Coding agent for feature #{feature_id} finished",
        ...
        try:
            ...
            debug_log.log(
                "DB",
                f"Feature #{feature_id} state after session.expire_all()",
        ...
        if return_code != 0:
            ...
            if failure_count >= MAX_FEATURE_RETRIES:
                debug_log.log("COMPLETE", f"Feature #{feature_id} exceeded max retries", failure_count=failure_count)
    ...
    def stop_feature(self, feature_id: int) -> tuple[bool, str]:
        ...
        if proc:
            ...
            debug_log.log(
                "STOP",
                f"Killed feature {feature_id} process tree",
                status=result.status,
                children_found=result.children_found,
                children_terminated=result.children_terminated,
    ...
    def stop_all(self) -> None:
        ...
        for feature_id, proc in testing_items:
            ...
            debug_log.log(
                "STOP",
                f"Killed testing agent for feature #{feature_id} (PID {proc.pid})",
                status=result.status,
                children_found=result.children_found,
                children_terminated=result.children_terminated,
                children_killed=result.children_killed,
    ...
    async def run_loop(self):
        ...
        debug_log.log(
            "STARTUP",
            "Orchestrator run_loop starting",
        ...
        if not has_features(self.project_dir):
            ...
            try:
                ...
                debug_log.log(
                    "INIT",
                    "Post-initialization database state",
                    max_concurrency=self.max_concurrency,
                    yolo_mode=self.yolo_mode,
                    testing_agent_ratio=self.testing_agent_ratio,
            ...
            try:
                ...
                for i, feature in enumerate(features_to_start):
                    ...
                    if not success:
                        debug_log.log(
                            "SPAWN",
                            f"FAILED to start feature #{feature['id']}",
                        ...
                        debug_log.log(
                            "SPAWN",
                            f"Successfully started feature #{feature['id']}",
                            feature_name=feature["name"],
